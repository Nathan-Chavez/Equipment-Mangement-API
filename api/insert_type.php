<?php
	...
	$sql="SELECT EXISTS(SELECT * from `type` where `type` = '$name')";
	$result=$dblink->query($sql) or
			die("Something went wrong with $sql<br>".$dblink->error);
	$row = $result->fetch_row();
	if($row[0] != 1)
	{
		$sql="Insert into `type` (`type`, `status`) values('$name','$status')";
		$result=$dblink->query($sql) or
				die("Something went wrong with $sql<br>".$dblink->error);
